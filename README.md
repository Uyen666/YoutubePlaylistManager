...
4. **🤖 一鍵在 YouTube 建立播放清單 / 在 Bilibili 建立收藏夾**：
   - 每個分類卡片支援一鍵「**➕ 建立清單**」(YouTube) 或「**➕ 建立收藏**」(Bilibili)。
   - **YouTube 原生引擎**：透過本地 Innertube API 與 SAPISIDHASH 認證，秒速建立全新播放清單、設定隱私度（私人 / 不公開 / 公開）並自動批次加入影片。
   - **Bilibili 原生引擎**：透過 B 站 Web API 與 CSRF (bili_jct) 認證，支援 BVID/AID 自動轉換與批次加入，一鍵在 B 站個人空間建立對應名稱之收藏夾！
...
5. **檢視、微調與建立/匯出**：
   ...
   - **一鍵建立 YouTube 清單 / B 站收藏夾**：點擊分類卡片右上角的「➕ 建立清單」(YouTube) 或「➕ 建立收藏」(Bilibili)，自動在帳號建立全新播放清單/收藏夾並加入該分類之所有影片。
